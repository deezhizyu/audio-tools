...
describe('buildEnergyHistogram', () => {
  test('sums arrivals into the correct time bin per band, keeping their absolute energy', () => {
    // panPosition is irrelevant to this function's band summation — it is covered separately below — so it
    // is set to an arbitrary value here.
    const arrivals: ImpulseArrival[] = [
      { timeSeconds: 0.001, energy: { low: 1, mid: 2, high: 3 }, panPosition: 0 },
      { timeSeconds: 0.004, energy: { low: 1, mid: 1, high: 1 }, panPosition: 0 }, // same 5ms bin as the arrival above
      { timeSeconds: 0.006, energy: { low: 4, mid: 0, high: 0 }, panPosition: 0 }, // next bin
    ];
  ...
  test('drops arrivals outside the requested total duration', () => {
    const arrivals: ImpulseArrival[] = [{ timeSeconds: 5, energy: { low: 1, mid: 1, high: 1 }, panPosition: 0 }];
    const histogram = buildEnergyHistogram(arrivals, 0.005, 0.02);
  ...
  test("a bin's pan position is the energy-weighted average of its arrivals' pan positions", () => {
    const arrivals: ImpulseArrival[] = [
      { timeSeconds: 0.001, energy: { low: 1, mid: 0, high: 0 }, panPosition: -1 }, // full left, weight 1
      { timeSeconds: 0.002, energy: { low: 3, mid: 0, high: 0 }, panPosition: 1 }, // full right, weight 3
    ];
  ...
  test('a single, fully left-panned arrival gives its bin a pan position of exactly -1', () => {
    const arrivals: ImpulseArrival[] = [{ timeSeconds: 0.001, energy: { low: 1, mid: 1, high: 1 }, panPosition: -1 }];
    const histogram = buildEnergyHistogram(arrivals, 0.005, 0.01);
