...
/** 0.1/0.1/0.15 (bare concrete/tile) made a freshly-drawn room, before anyone touches the per-object absorption
    ...
    with reverb" rather than "an empty tiled hallway." Existing objects keep whatever absorption they were drawn
    with — this only changes what new objects start at. Also lives on only as the `generic-object`/
    `generic-absorber` catalog entries' baseline absorption, for the same reason as `SCATTER_AMOUNT` above. */
