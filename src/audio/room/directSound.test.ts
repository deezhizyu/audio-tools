...
function buildWall(x: number): RoomBox {
  return { id: 'wall', kind: 'wall', x, y: -1, z: -1, width: 0.5, height: 2, depth: 2, absorption: { low: 0.1, mid: 0.1, high: 0.1 } };
}
...
describe('computeDirectSoundPath', () => {
  ...
  test('is occluded by a wall standing between source and listener', () => {
    const scene: RoomScene = { boxes: [buildWall(2)], source: { x: 0, y: 0, z: 0 }, listener: { x: 5, y: 0, z: 0 } };
    expect(computeDirectSoundPath(scene).isOccluded).toBe(true);
  });

  test('is not occluded by a wall behind the listener', () => {
    const scene: RoomScene = { boxes: [buildWall(10)], source: { x: 0, y: 0, z: 0 }, listener: { x: 5, y: 0, z: 0 } };
  ...
  test('treats a source and listener at the same point as unoccluded with zero distance', () => {
    const scene: RoomScene = { boxes: [buildWall(0)], source: { x: 1, y: 1, z: 1 }, listener: { x: 1, y: 1, z: 1 } };
    const path = computeDirectSoundPath(scene);
