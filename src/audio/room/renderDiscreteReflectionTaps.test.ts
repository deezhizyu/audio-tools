...
describe('renderDiscreteReflectionTaps', () => {
  test("places a band-colored impulse at the arrival's exact delay sample, identically on every channel when jitter is zero", () => {
    const arrival: ImpulseArrival = { timeSeconds: 0.01, energy: { low: 1, mid: 1, high: 1 }, bounceOrder: 0 };
    const channels = buildChannels(50);

    renderDiscreteReflectionTaps(channels, [arrival], 1000, 1, FIXED_RANDOM_SOURCE);
  ...
  test('produces no impulse at all for a silent (zero-energy) arrival', () => {
    const arrival: ImpulseArrival = { timeSeconds: 0.01, energy: { low: 0, mid: 0, high: 0 }, bounceOrder: 0 };
    const channels = buildChannels(50);
  ...
  test('decorrelates channels when given a varying random source', () => {
    const arrivals: ImpulseArrival[] = [
      { timeSeconds: 0.01, energy: { low: 1, mid: 1, high: 1 }, bounceOrder: 0 },
      { timeSeconds: 0.012, energy: { low: 1, mid: 1, high: 1 }, bounceOrder: 0 },
      { timeSeconds: 0.015, energy: { low: 1, mid: 1, high: 1 }, bounceOrder: 0 },
    ];
    const channels = buildChannels(50);

    renderDiscreteReflectionTaps(channels, arrivals, 1000, 1, buildCyclingRandomSource());
  ...
  test('silently drops an arrival whose delay falls outside the channel bounds', () => {
    const arrivals: ImpulseArrival[] = [
      { timeSeconds: 10, energy: { low: 1, mid: 1, high: 1 }, bounceOrder: 0 }, // far past the end of a 50-sample channel at 1000Hz
    ];
    const channels = buildChannels(50);

    expect(() => renderDiscreteReflectionTaps(channels, arrivals, 1000, 1, FIXED_RANDOM_SOURCE)).not.toThrow();
    for (const channel of channels) {
      expect(Array.from(channel).every(value => value === 0)).toBe(true);
    }
  });
